...
async function fetchProductos() {
  console.log("🔄 Cargando productos...", { page: state.page, pageSize: state.pageSize, rol: state.rol, search: state.search, categoriaFilter: state.categoriaFilter, semaforoFilter: state.semaforoFilter });
  try {
    ...
    // Aplicar filtro de semáforo en el frontend (ya que se calcula dinámicamente)
    let productosFiltered = data.data;
    if (state.semaforoFilter) {
      productosFiltered = data.data.filter(p => p.semaforo === state.semaforoFilter);
    }

    state.productos = productosFiltered;
    state.total = state.semaforoFilter ? productosFiltered.length : data.total;
...
console.log("🚀 Inicializando aplicación MiSalud Precios...");
console.log("📍 Verificando elementos del DOM...");
console.log("- tableBody:", tableBody ? "✅" : "❌");
console.log("- rolSelect:", rolSelect ? "✅" : "❌");
console.log("- listasContent:", listasContent ? "✅" : "❌");
